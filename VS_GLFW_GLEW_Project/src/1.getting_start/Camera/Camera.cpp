...
void DrawSquare_texture(GLFWwindow* window, unsigned int* textures)
{
	...
	Shader ourShader("./src/1.getting_start/Camera/shader.vs", "./src/1.getting_start/Camera/shader.fs");
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		for (unsigned int i = 0; i < 10; i++)
		{
			...
			unsigned int ratio = (i + 1) % 3;
			float angle = 20.0f * (ratio == 1 ? 1 : 0);
			model = glm::rotate(model, (float)glfwGetTime() * glm::radians(angle), glm::vec3(0.1f, 0.3f, 0.5f));
			ourShader.SetMatrix4fv("model", glm::value_ptr(model));
...
void mouse_callback(GLFWwindow* window, double xposIn, double yposIn) 
{
	...
	xoffset *= sensitiveity;
	yoffset *= sensitiveity;
